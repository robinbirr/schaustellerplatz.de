      ...
      for (const file of htmlFiles) {
        if (file.endsWith('.html')) {
          ...
          for (const element of elements) {
            ...
            const normalizedVendorPath = vendorPath
              .replace(/\//g, path.sep)
              .replace(/\\/g, path.sep)
            const relativePathFromVendor =
              fullPath.split(normalizedVendorPath)[1]
...
const distribute = async () => {
  try {
    ...
    await fs.ensureDir(`${config.path.dist}/${config.path.json}`)
    ...
    await fs.copy(
      `${config.path.json}`,
      `${config.path.dist}/${config.path.json}`
    )
